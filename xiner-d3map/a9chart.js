...
d3.json("world-110m.geojson").then(function (json) {
    ...
    d3.json("a9data.json").then(function (data) {
        data.forEach(function (d) {
            countryWaste[d.country] = +d.population;
        });
        ...
        worldMap.append("circle")
            ...
            .attr("r", function (d) {
                return countryWaste[d.properties.name] / 2 || 0;
            })
            .attr("fill", "red")
            .attr("opacity", 0.7)
        ...
        worldMap.append("text")
            ...
            .attr("x", function (d) {
                return countryWaste[d.properties.name] / 2 + 2 || 0;
            })
            .attr("alignment-baseline", "middle")
            .attr("fill", "black")
            .text(function (d) {
                return countryWaste[d.properties.name] ? d.properties.name : "";
            });
    ...
    legendLabel.append("circle")
        .attr("transform", function (d, i) {
            return "translate(" + rectWidth / 2 + "," + (rectHeight - 10 - d / 2) + ")";
        })
        .attr("r", function (d) {
            return d / 2
        })
    ...
    legendLabel.append("text")
        .attr("transform", function (d, i) {
            return "translate(" + rectWidth / 2 + "," + (rectHeight - 10 - d) + ")";
        })
    ...
    d3.json("a9data.json").then(function (data) {
        ...
        worldMap.append("path")
            .attr("fill", function (d) {
                return countryWaste[d.properties.name] ? colorScale(countryWaste[d.properties.name]) : "lightgrey"
            })
            .attr("stroke", "white")
            .attr("d", path)  //generate geographic path
            .on("mouseover", function (d) {
